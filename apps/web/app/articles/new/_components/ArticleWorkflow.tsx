...
type WorkflowStep = 'start' | 'planning' | 'review_strategy' | 'generate_body' | 'finalize' | 'completed';
...
export function ArticleWorkflow() {
  ...
  if (step === 'generate_body' && strategy?.outline) {
    ...
    const progress = ((currentSectionIndex / strategy.outline.length) * 100).toFixed(0);
        ...
        <div className="bg-gray-50 p-6 rounded-lg space-y-4">
          ...
          {sections[currentSectionIndex] ? (
            <div className="prose max-w-none">
              <p className="whitespace-pre-wrap">{sections[currentSectionIndex].body}</p>
            </div>
          ) : (
        ...
        <div className="bg-white border rounded-lg p-8 space-y-6">
          ...
          {sections.map((section, idx) => (
            <div key={idx} className="space-y-2">
              ...
              <p className="whitespace-pre-wrap text-gray-700">{section.body}</p>
            </div>
          ))}
        </div>
