...
export function deduplicateBlocks(
  ...
  const sortedBlocks = [...blocks].sort((a, b) => {
    ...
    const aConfidence =
      a.lines.reduce((sum, line) => {
        const lineConf =
          line.tokens.reduce((s, t) => s + (t.confidence || 0), 0) /
          Math.max(1, line.tokens.length);
        return sum + lineConf;
      }, 0) / Math.max(1, a.lines.length);

    const bConfidence =
      b.lines.reduce((sum, line) => {
        const lineConf =
          line.tokens.reduce((s, t) => s + (t.confidence || 0), 0) /
          Math.max(1, line.tokens.length);
        return sum + lineConf;
...
export function mergeNormalizedOcrPages(
  ...
  for (const [pageIndex, pages] of Array.from(pagesByIndex.entries()).sort(
    (a, b) => a[0] - b[0]
  )) {
  ...
  return {
    pages: mergedPages,
    metadata: {
      processingTime: tileOcrResults.reduce(
...
export function calculateOptimalTileSize(
  ...
  if (pageArea < maxTileSizePx * maxTileSizePx * 2) {
    return Math.max(
      minTileSizePx,
      Math.min(maxTileSizePx, Math.max(pageWidthPx, pageHeightPx))
    );
  }

  // For large pages, calculate optimal tile size
  const targetTiles = Math.ceil(
    Math.sqrt(pageArea / (maxTileSizePx * maxTileSizePx))
  );
